...
function renderRobotViewer(model: import('./robotWireframe').WireframeModel): string {
  ...
  const src = [
    ...
    '  g.stroke();}',
    ' var LB=M.labels||[];',
    ' for(var i=0;i<LB.length;i++){var Lb=LB[i];',
    // Normal-tagged labels (bumper numbers) show only on camera-facing sides,
    // centered and scaled with distance so they read as painted-on text.
    '  if(Lb.n&&((Lb.pos[0]-camp[0])*Lb.n[0]+(Lb.pos[1]-camp[1])*Lb.n[1]+(Lb.pos[2]-camp[2])*Lb.n[2])>0)continue;',
    '  var lq=pr(Lb.pos);if(!lq)continue;g.fillStyle=Lb.color;',
    '  if(Lb.n){var dz=(Lb.pos[0]-camp[0])*fwd[0]+(Lb.pos[1]-camp[1])*fwd[1]+(Lb.pos[2]-camp[2])*fwd[2];',
    '   var fs=Math.max(8,Math.min(40,0.09*fl/dz));g.font="bold "+fs.toFixed(1)+"px system-ui";',
    '   g.textAlign="center";g.fillText(Lb.text,lq[0],lq[1]+fs*0.35);g.textAlign="left";}',
    '  else{g.font="bold 12px ui-monospace,Menlo,monospace";g.fillText(Lb.text,lq[0]+6,lq[1]);}}',
