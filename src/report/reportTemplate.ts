...
function renderRobotViewer(model: import('./robotWireframe').WireframeModel): string {
  ...
  const src = [
    ...
    ' for(var i=0;i<F.length;i++){var f=F[i];var zs=0,ok=true,pp=[];',
    '  for(var j=0;j<f.pts.length;j++){var p=f.pts[j];var d0=p[0]-camp[0],d1=p[1]-camp[1],d2=p[2]-camp[2];',
    '   var z=d0*fwd[0]+d1*fwd[1]+d2*fwd[2];if(z<0.05){ok=false;break;}zs+=z;',
    '   pp.push([w/2+(d0*right[0]+d1*right[1])*fl/z,h/2-(d0*up[0]+d1*up[1]+d2*up[2])*fl/z]);}',
    '  if(ok)order.push([zs/f.pts.length,f,pp]);}',
    ' order.sort(function(a,b){return b[0]-a[0];});',
    ' for(var i=0;i<order.length;i++){var f=order[i][1],pp=order[i][2];',
    '  g.fillStyle=rgba(f.color,f.alpha);g.beginPath();g.moveTo(pp[0][0],pp[0][1]);',
    '  for(var j=1;j<pp.length;j++)g.lineTo(pp[j][0],pp[j][1]);g.closePath();g.fill();}',
    // Text decals baked onto planar quads (bumper numbers): map a 100x30
    // texture space onto the projected face with a canvas transform, so the
    // text shears and rotates with the surface. Top-left origin keeps the
    // glyphs upright; the outward normal culls back-facing sides.
    ' var DC=M.decals||[];',
    ' for(var i=0;i<DC.length;i++){var D=DC[i];',
    '  if((D.quad[0][0]-camp[0])*D.n[0]+(D.quad[0][1]-camp[1])*D.n[1]+(D.quad[0][2]-camp[2])*D.n[2]>0)continue;',
    '  var d0=pr(D.quad[0]),d2=pr(D.quad[2]),d3=pr(D.quad[3]);if(!d0||!d2||!d3)continue;',
    '  g.save();g.transform((d2[0]-d3[0])/100,(d2[1]-d3[1])/100,(d0[0]-d3[0])/30,(d0[1]-d3[1])/30,d3[0],d3[1]);',
    '  g.fillStyle=D.color;g.font="bold 20px system-ui";g.textAlign="center";g.textBaseline="middle";',
    '  g.fillText(D.text,50,15,96);g.restore();}',
    ' g.lineWidth=1.4;',
